...
import { ThemeContext } from '../ThemeContext.js';  // Correct path based on your structure
import axios from 'axios';
import './Collections.css';  // Include styles for the collection
...
const CarCollection = () => {
    ...
    useEffect(() => {
        const fetchCars = async () => {
            try {
                const response = await axios.get('https://universal-dynamics-backend.onrender.com/vintage-cars');
   ...
   useEffect(() => {
        const filtered = cars.filter(car => {
            ...
            return brand.includes(searchValue.toLowerCase()) || model.includes(searchValue.toLowerCase());
        });
    ...
    return (
        <div ref={collectionRef} className="car-collection">
            <h2 className="heading">Vintage Car</h2>
